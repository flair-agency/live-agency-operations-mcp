...
import { createCreatorScoutingMcpServer } from "./creator-scouting-mcp-server.mjs";
import {
...
export async function main() {
  ...
  const bindingProfile = resolveProviderBindingProfile(
    ...
    {
      profileId: "flair-creator-scouting-read",
      domain: "creator-scouting",
      authority: "read",
    },
  );
  const server = createCreatorScoutingMcpServer({
    runtime: createOperationsRuntime({ rootDir, bindingProfile }),
  });
  await server.connect(new StdioServerTransport());
  console.error("creator-scouting MCP server running on stdio with read authority");
}
